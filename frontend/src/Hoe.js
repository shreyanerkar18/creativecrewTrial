...
const Hoe = () => {
  ...
  const onClickingDay = (day) => {
    if (isSeatsChanging && !isAddingManager) {
      setSelectedManager({
        ...selectedManager,
        seats_array: {
          ...selectedManager.seats_array,
          [selectedDay]: selectedSeats,
        },
        seats_data: {
          ...selectedManager.seats_data,
          [selectedDay]: selectedSeats,
        },
      });
        ...
        for (let i = seat; i < seat + count; i++) {
          if (
            ...
            managers.some((manager) =>
              manager.seats_array[selectedDay].includes(i)
            )
          )
            break;
  ...
  const onClickingChangeSeats = () => {
    ...
    setSelectedSeats([
      ...selectedSeats,
      ...selectedManager.seats_array[selectedDay],
    ]);
  };
      ...
      if (!firstName || !lastName || seatCount <= 0) {
        ...
        try {
          ...
          setSelectedManager({
            ...
            seats_array: newManager.seats_data,
          });
        } catch (err) {
          console.error(err);
        }
  ...
  const countAllocatedSeats = () => {
    const seats = managers.flatMap(
      (manager) => manager.seats_array[selectedDay]
    );
    const filteredSeats =
      isSeatsChanging && !isAddingManager
        ? seats.filter(
            (item) => !selectedManager.seats_array[selectedDay].includes(item)
          )
        : seats;
